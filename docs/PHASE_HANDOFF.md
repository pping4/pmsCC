# Phase Handoff — Receipt Standardization → Card Settlement → Phase 6 Cleanup → Lazy-Invoice Backfill

> **Branch:** `feat/receipt-standardization` (38 commits ahead of `feat/consolidation`)
> **Status:** All 5 phases + the entire Phase 6 cleanup + Phase 6.7–6.11 invoice-visibility chain complete + verified.
> **Last commit:** `40def7e fix(add-service): Phase 6.11 — always cut INV-EX when "ลงบิลไว้ก่อน"`
...
This document is the canonical source-of-truth for everything that happened between commits `dab7d09` (Phase 1 start) and `40def7e` (Phase 6.11 end). Future agents must read sections **1 → 4** before making changes; section 5 documents what shipped in Phase 6.
...
| 6.7 | **Extend "เก็บเงินภายหลัง" cuts INV-EX** | Hoisted `createInvoiceFromFolio` out of the `collectNow` guard in `/api/bookings/[id]/extend` so the extension always lands as a real invoice (status=unpaid when pay-later, paid when collected) — bill tab + collection center can see it immediately | `9a8cc2f` |
| 6.8 #1 | **Bill-tab per-invoice pay** | `DetailPanel` switched from `billingPayOpen: boolean` to `billingPayInvoiceId: string\|null` so only the clicked row's form opens. `/api/bookings/[id]/pay` accepts optional `invoiceId` to restrict allocation to that one invoice; omitting it keeps the legacy "pay all outstanding" path (FolioLedger quick-pay) alive | `dce1253` |
| 6.8 #2 | **Checkout settles ALL unpaid invoices** | `/api/checkout` queries every unpaid invoice on the booking (existing INV-GN/INV-EX + the new INV-CO) and pays them in ONE Payment with FIFO allocations; receipt aggregates items across all paid invoices; single ledger pair `DR Cash N / CR AR N` | `708b7f7` |
| 6.9 | **Check-in without prepay still cuts INV-CI** | Same hoist as 6.7: `/api/checkin` always creates INV-CI (status=unpaid when no upfront pay) so a walk-in booking's stay charges are visible in the bill tab from the start, instead of sitting as UNBILLED rows that the proforma fallback hid once any other invoice existed | `8fcecdd` |
| 6.10 | **Bill-tab button respects overdue + partial** | `/api/billing/collection` auto-flips `unpaid → overdue` past dueDate, but the pay button checked `status === 'unpaid'` exactly — overdue rows lost the button. Replaced with a derived `isInvoiceCollectible(inv)` check using `grandTotal > paidAmount` + status whitelist (excludes voided/cancelled/paid). Also adds explicit `overdue` style + fixes `isPaid` to no longer treat `partial` as fully paid | `1ea5fd3` |
| 6.11 | **Add-service "ลงบิลไว้ก่อน" cuts INV-EX** | Last instance of the same lazy-invoice bug — `/api/bookings/[id]/add-service` nested the invoice creation inside `if (collectNow)`. Hoisted out: every minibar / laundry / etc. row gets INV-EX immediately. Closes the bill-tab visibility loop end-to-end | `40def7e` |

All phases have ledger postings that produce a **trial balance that reconciles**. Seven E2E harnesses verify this; do not regress these without re-running them.

### The lazy-invoice pattern (Phase 6.7–6.11)

Five flows were skipping invoice creation when the cashier picked the
"defer billing" / "pay later" branch, leaving `FolioLineItem` rows with
`billingStatus=UNBILLED` and no Invoice attached. The bill tab reads from
Invoice rows (not raw line items), so the unbilled charges were invisible
until checkout bundled them into INV-CO. Fix shape is identical in every
case:
...
if (collectNow && totalAmount > 0 && paymentMethod) {
  ...
  // mark paid + create payment
}

// AFTER
const invResult = totalAmount > 0
  ? await createInvoiceFromFolio(tx, ...)   // ALWAYS cut the invoice
  : null;
if (collectNow && invResult && paymentMethod) {
  // mark paid + create payment              ← payment block stays optional
}
```

Routes touched: `extend`, `checkin`, `add-service`. New routes should
follow this pattern: **invoice creation is decoupled from payment
collection**. Cashier sees every charge in the bill tab; payment is a
later action.
...
| **`scripts/e2e-extend-pay-later.ts`** | **Extend collectNow=false cuts INV-EX + follow-up payment (Phase 6.7)** | **21** |
| **`scripts/e2e-bill-tab-per-invoice.ts`** | **Pay specific invoice via `invoiceId` scope (Phase 6.8 #1)** | **14** |
| **`scripts/e2e-checkout-multi-invoice.ts`** | **Checkout settles ALL unpaid invoices in one Payment (Phase 6.8 #2)** | **13** |
| **`scripts/e2e-checkin-no-prepay-invoice.ts`** | **Check-in without upfront pay still cuts INV-CI (Phase 6.9)** | **16** |
| **`scripts/e2e-add-service-bill-later.ts`** | **Add-service collectNow=false cuts INV-EX (Phase 6.11)** | **21** |
...
### 5.7 Lazy-invoice chain — ✅ shipped (`9a8cc2f`, `8fcecdd`, `40def7e`)

User-reported series — every flow that lets the cashier "defer billing"
(extend pay-later, walk-in no-prepay check-in, add-service "ลงบิลไว้ก่อน")
was leaving `FolioLineItem` rows as UNBILLED with no invoice. The bill
tab reads Invoice rows, so the proforma fallback hid the charges as soon
as any other real invoice existed, and the cashier had no row to collect
against until checkout bundled them into INV-CO.

Fix is identical in all three routes (see "lazy-invoice pattern" above):
hoist `createInvoiceFromFolio` out of the `collectNow` guard so every
charge action always cuts an invoice (status=unpaid when pay-later).

Each route also has its dedicated E2E:
- `scripts/e2e-extend-pay-later.ts` — 21 assertions
- `scripts/e2e-checkin-no-prepay-invoice.ts` — 16 assertions
- `scripts/e2e-add-service-bill-later.ts` — 21 assertions

### 5.8 Bill-tab UX fixes — ✅ shipped (`dce1253`, `708b7f7`, `1ea5fd3`)

Three companion fixes that landed alongside the lazy-invoice work:

- **6.8 #1 — Per-invoice pay (`dce1253`).** `DetailPanel` previously
  shared a single `billingPayOpen: boolean` across every unpaid card,
  so clicking 💳 รับชำระเงิน on one row expanded the form on every
  row. Server side `/api/bookings/[id]/pay` ignored which row was
  clicked and allocated the booking total across all unpaid invoices.
  Fixed: state is now `billingPayInvoiceId: string | null` per row;
  API accepts optional `invoiceId` to scope allocation.
- **6.8 #2 — Checkout pays everything (`708b7f7`).** `/api/checkout`
  only created INV-CO and paid that one invoice — prior unpaid
  INV-GN/INV-EX rows stayed unpaid forever, AR carried a residual.
  Fixed: after creating INV-CO, query every unpaid invoice on the
  booking and pay all of them in ONE Payment with FIFO allocations.
- **6.10 — Pay button respects overdue/partial (`1ea5fd3`).** The
  pay button checked `inv.status === 'unpaid'` exactly, but
  `/api/billing/collection` auto-flips `unpaid → overdue` past
  dueDate. Overdue rows lost the button even though they still
  showed "ค้างชำระ" via the badge fallback. Replaced with
  `isInvoiceCollectible(inv)` = `grandTotal > paidAmount` + status
  whitelist. Companion fix: `isPaid` no longer treats `partial` as
  paid.

E2Es:
- `scripts/e2e-bill-tab-per-invoice.ts` — 14 assertions
- `scripts/e2e-checkout-multi-invoice.ts` — 13 assertions

---

### Backfill helper (one-off)

`scripts/_backfill-unbilled-invoices.mjs` — finds folios with orphan
UNBILLED rows (left behind by the pre-6.7/6.9/6.11 versions of the
routes) and cuts one INV-EX per folio for them. Run with `--apply` to
commit; default is dry-run. Idempotent — a clean DB returns "no
UNBILLED orphans found". Use this if production has been on an older
build and you need to surface the orphans without resetting test data.
...
_End of handoff. Phase 6 core shipped 2026-05-11; Phase 6.7–6.11 lazy-invoice + bill-tab UX chain shipped 2026-05-13. Update this file when a new phase opens._
